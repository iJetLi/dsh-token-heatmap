...
- **0.4.1（fork 会话不再重复计入父会话用量）**：DSH 的 fork 子会话（header `isSeeded=true`）日志以父会话事件的完整复制开头，其前 `inheritedEventCount` 个事件是**父会话**的 usage（父会话折叠时已计入）。此前折叠从 seq 0 读整份日志，同一批 token 被计两次——实测 2026-09-14 由 8.34 亿虚增到 13.15 亿。现在两条折叠路径（`collectUsage` 的 live 折叠、`session/event` 实时监听）都从 fork 切点开始，用官方 `session.inheritedEventCount`（stored 日志不再读取，故不再需要从 `session/end-seed` 推导切点）。**resume 不是 fork**：`isSeeded=false` 的会话种子是它自己的历史，仍整份折叠。缓存格式版本为 2，旧缓存（可能含重复计入的天数）会被丢弃重建。
...
- **0.4.4（修复热力图端点挂起）**：stored 会话**只枚举 id、不再读取日志**。两种 `sessionPersistence` 读取接口（`readFrom()` / `open()`+`read()`）都返回**整份日志**，在几百个 stored 会话 / 数百 MB 历史下，每次请求都要全部重读一遍，导致热力图端点**永久挂起**（UI 一直"加载中…"）、进程 CPU 打满，并**拖垮同进程的其他插件**（设置 → 使用统计 报 `signal timed out`，且首次正常、之后超时——因为它也在这台机器上被饿着）。`dsh-usage` 之所以从不卡顿，正是因为它**完全不碰 `sessionPersistence`**，只折 `session/event` 实时流；本插件现采用同样形状。代价：**早于本次启动的历史天数不再回填**（已折叠进缓存的天数照常显示），实时用量不受影响。
